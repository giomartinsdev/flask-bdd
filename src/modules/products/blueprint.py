...
from modules.products.service import ProductError, ProductService
...
def _make_service() -> ProductService:
    ...
    sqs = boto3.client("sqs", endpoint_url=os.environ.get("AWS_ENDPOINT_URL"), region_name="us-east-1")
    sns = boto3.client("sns", endpoint_url=os.environ.get("AWS_ENDPOINT_URL"), region_name="us-east-1")
    s3 = boto3.client("s3", endpoint_url=os.environ.get("AWS_ENDPOINT_URL"), region_name="us-east-1")
...
def list_products():
    ...
    svc = _make_service()
    products = svc.list_all(ProductFilters(
...
def update_stock(product_id: int):
    ...
    svc = _make_service()
    try:
        product = svc.update_stock(product_id, data["stock"])
    except ProductError as e:
        return _error(str(e), e.status)
    return jsonify(product.to_dict())
...
def toggle_status(product_id: int):
    svc = _make_service()
    try:
        product = svc.toggle_status(product_id)
    except ProductError as e:
        return _error(str(e), e.status)
    return jsonify(product.to_dict())
